...
const name = "USER";
const secretKey = Deno.env.get(`${name}`);
...
const dataToEncrypt = "Hemlig text som ska krypteras";
if (secretKey) {
    const encryptedData = await encrypt(dataToEncrypt, secretKey);
    console.log("Krypterad data:", encryptedData);
}
    ...
    const newRecord = await Promise.all(records.map(async (record) => {
        for (let i = 0; i < record.recipients.length; i++) {
            ...
            if (recipient.year === year) {
                const data = await encrypt(recipient.recipient, secrets.find(x => x.name ===record.name)!.secretKey)
                record.recipients[i] = {...recipient, encrypted: true, recipient: data }
            }
        }
        return record;
    }))
